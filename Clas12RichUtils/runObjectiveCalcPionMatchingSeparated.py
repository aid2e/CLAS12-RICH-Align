...
corresponding to one dataset (assumed to be primarily pi+ and pi- in the RICH)
'''

jobid = sys.argv[1]
align_sector = int(sys.argv[2])
output_dir = sys.argv[3]
# TODO: Need torus flag to know which pion is inbending/outbending
...
def calc_cher_residual_generic(
    ...
    for c in tiles:
        ...
        # per-photon beta
        p_evt   = p[mask]
        mass    = 0.13957039  # TODO: Some argument for non-pions, in case they are ever used
...
pid_inb = 211
pid_outb = -211
# Collect single-photon cherenkov angle distributions separated by layer/tile/topology
file_pions_inb = output_dir+f"/rich/log/root_files/output_{jobid}.root"
print("starting direct")
cher_dist_direct_inb    = calc_cher_residual_layers_generic(file_pions_inb, tiles_by_layer, scale_angle=True,
                                                                mode="direct",
                                                                ebpid_select=pid_inb)
cher_dist_direct_outb    = calc_cher_residual_layers_generic(file_pions_inb, tiles_by_layer, scale_angle=True,
                                                                mode="direct",
                                                                ebpid_select=pid_outb)

mirrors_row1 = [21,25,22]
mirrors_row2 = [28,29,30,23,26,27,24]

cher_dist_spher_row1_inb  = defaultdict(dict)
cher_dist_spher_row2_inb  = defaultdict(dict)
cher_dist_spher_row1_outb  = defaultdict(dict)
cher_dist_spher_row2_outb  = defaultdict(dict)

mirrors_planar = [11,14,15,16,17] # 1 reflection topology planar mirrors
cher_dist_planar_inb  = defaultdict(dict)
cher_dist_planar_outb  = defaultdict(dict)

for m in mirrors_planar:
    print(m)
    print("ele")
    cher_dist_planar_inb[m] = calc_cher_residual_layers_generic(
            file_pions_inb, tiles_by_layer, planar_mirror=m, ebpid_select=pid_inb, scale_angle=True,
            mode="planar"
        )
    cher_dist_planar_outb[m] = calc_cher_residual_layers_generic(
            file_pions_inb, tiles_by_layer, planar_mirror=m, ebpid_select=pid_outb, scale_angle=True,
            mode="planar"
        )
...
for m in mirrors_row1:
    for p in [12,13]:
        print(m,p)
        cher_dist_spher_row1_inb[m][p] = calc_cher_residual_layers_generic(
            file_pions_inb, tiles_by_layer, planar_mirror=p, spherical_mirror=m,
            ebpid_select=pid_inb, scale_angle=True,
            mode="spherical"
        )
        cher_dist_spher_row1_outb[m][p] = calc_cher_residual_layers_generic(
            file_pions_inb, tiles_by_layer, planar_mirror=p, spherical_mirror=m,
            ebpid_select=pid_outb, scale_angle=True,
            mode="spherical"
        )
...
for m in mirrors_row2:
    for p in [13]:
        print(m,p)
        cher_dist_spher_row2_inb[m][p] = calc_cher_residual_layers_generic(
            file_pions_inb, tiles_by_layer, planar_mirror=p, spherical_mirror=m,
            ebpid_select=pid_inb, scale_angle=True,
            mode="spherical"
        )
        cher_dist_spher_row2_outb[m][p] = calc_cher_residual_layers_generic(
            file_pions_inb, tiles_by_layer, planar_mirror=p, spherical_mirror=m,
            ebpid_select=pid_outb, scale_angle=True,
            mode="spherical"
        )

cher_dist_spher_row1_inb = dict(cher_dist_spher_row1_inb)
cher_dist_spher_row2_inb = dict(cher_dist_spher_row2_inb)

cher_dist_spher_row1_outb = dict(cher_dist_spher_row1_outb)
cher_dist_spher_row2_outb = dict(cher_dist_spher_row2_outb)

##############################
# Functions for spread
# amongst topologies metric.
##############################

# for width of distribution
def iqr(arr):
...
def getSpreadMetric(
        ...
        *,
        ...
        spherical_shift = 3.5, # expected difference in mean cherenkov angle btw 2 refl and 0 refl
        spherical_shift_203 = 3.5, # expected difference in mean cherenkov angle btw 2 refl and 0 refl, 6cm aero. layer
):
...
distance_sum_outb = {201:0, 202:0, 203:0}
total_terms_outb = {201:0, 202:0, 203:0}
distance_sum_inb = {201:0, 202:0, 203:0}
total_terms_inb = {201:0, 202:0, 203:0}
terms = {}
layer_avg_inb = ufloat(0.0,0.0)
layer_avg_outb = ufloat(0.0,0.0)
for layer in tiles_by_layer.keys():
    for tile in tiles_by_layer[layer]:
        print(layer,tile)
        spread_together_inb = getSpreadMetric(
            cher_dist_direct_inb,cher_dist_planar_inb,cher_dist_spher_row1_inb,cher_dist_spher_row2_inb,
            layer=layer, tile=tile,
            bounds=(280, 350),
            min_counts=1000,
        )
        spread_together_outb = getSpreadMetric(
            cher_dist_direct_outb,{}, # don't use planar for outbending pions
            cher_dist_spher_row1_outb,cher_dist_spher_row2_outb,
            layer=layer, tile=tile,
            bounds=(280, 350),
            min_counts=1000,
        )

        if spread_together_inb>0:
            distance_sum_inb[layer] += spread_together_inb
            total_terms_inb[layer] += 1
            layer_avg_inb += spread_together_inb
        if spread_together_outb>0:
            distance_sum_outb[layer] += spread_together_outb
            total_terms_outb[layer] += 1
            layer_avg_outb += spread_together_outb
    if total_terms_inb[layer] != 0:
        terms[f'topo_mismatch_layer{layer}_inb'] = layer_avg_inb/total_terms_inb[layer]
    else:
        terms[f'topo_mismatch_layer{layer}_inb'] = ufloat(0.0,0.0)

    if total_terms_outb[layer] != 0:
        terms[f'topo_mismatch_layer{layer}_outb'] = layer_avg_outb/total_terms_outb[layer]
    else:
        terms[f'topo_mismatch_layer{layer}_outb'] = ufloat(0.0,0.0)
    ...
    *,
    ...
    for ln in tiles_by_layer:
        for tn in tiles_by_layer[ln]:
            ...
            total_tiles += 1
    if total_tiles == 0:
        return ufloat(20.0/4.0,0.0) #"punish" if we had a topology we expected that somehow has no stats
    else:
        return total/total_tiles

# Get sum of tile-averaged IQR**2 for all topologies,
# separately for inbending and outbending
iqr_sum_inb = ufloat(0,0)
n_topos_inb = 0
for i in mirrors_row1:
    iqr_sum_inb += get_width_sum_alltiles(cher_dist_spher_row1_inb[i][13])
    n_topos_inb += 1
    iqr_sum_inb += get_width_sum_alltiles(cher_dist_spher_row1_inb[i][12])
    terms[f'iqr_sph_{i}_12_inb'] = get_width_sum_alltiles(cher_dist_spher_row1_inb[i][12])
    terms[f'iqr_sph_{i}_13_inb'] = get_width_sum_alltiles(cher_dist_spher_row1_inb[i][13])
    n_topos_inb += 1
for i in mirrors_row2:
    iqr_sum_inb += get_width_sum_alltiles(cher_dist_spher_row2_inb[i][13])
    terms[f'iqr_sph_{i}_13_inb'] = get_width_sum_alltiles(cher_dist_spher_row2_inb[i][13])
    n_topos_inb += 1
for i in [11,14,15,16,17]:
    iqr_sum_inb += get_width_sum_alltiles(cher_dist_planar_inb[i])
    terms[f'iqr_planar_{i}_inb'] = get_width_sum_alltiles(cher_dist_planar_inb[i])
    n_topos_inb += 1

iqr_sum_inb += get_width_sum_alltiles(cher_dist_direct_inb)
n_topos_inb+=1
terms[f'iqr_direct_inb'] = get_width_sum_alltiles(cher_dist_direct_inb)

iqr_sum_outb = ufloat(0,0)
n_topos_outb = 0
for i in mirrors_row1:
    iqr_sum_outb += get_width_sum_alltiles(cher_dist_spher_row1_outb[i][13])
    n_topos_outb += 1
    iqr_sum_outb += get_width_sum_alltiles(cher_dist_spher_row1_outb[i][12])
    terms[f'iqr_sph_{i}_12_outb'] = get_width_sum_alltiles(cher_dist_spher_row1_outb[i][12])
    terms[f'iqr_sph_{i}_13_outb'] = get_width_sum_alltiles(cher_dist_spher_row1_outb[i][13])
    n_topos_outb += 1
for i in mirrors_row2:
    iqr_sum_outb += get_width_sum_alltiles(cher_dist_spher_row2_outb[i][13])
    terms[f'iqr_sph_{i}_13_outb'] = get_width_sum_alltiles(cher_dist_spher_row2_outb[i][13])
    n_topos_outb += 1
#for i in [14,15,16,17]: # Skip planar for outbending pions
#    iqr_sum_outb += get_width_sum_alltiles(cher_dist_planar_outb[i])
#    terms[f'iqr_planar_{i}_outb'] = get_width_sum_alltiles(cher_dist_planar_outb[i])
#    n_topos_outb += 1

iqr_sum_outb += get_width_sum_alltiles(cher_dist_direct_outb)
n_topos_outb+=1
terms[f'iqr_direct_outb'] = get_width_sum_alltiles(cher_dist_direct_outb)

combined_metric = (distance_sum_inb[201] + distance_sum_inb[202] + distance_sum_inb[203] + 
                   distance_sum_outb[201] + distance_sum_outb[202] + distance_sum_outb[203] + 
                   iqr_sum_inb + iqr_sum_outb)
np.savetxt(output_dir+"/log/results/" + "rich-align-mobo-out_{}.txt".format(jobid),np.array([combined_metric.n,combined_metric.s]))

# Flatten individual terms for storage in csv
data = {}
