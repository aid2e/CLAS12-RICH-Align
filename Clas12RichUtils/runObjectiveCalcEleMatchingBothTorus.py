...
file_ele_inb = os.environ["OUTPUT_DIR"]+f"/rich/log/root_files/output_spherical_{jobid}_inb.root"
file_ele_outb = os.environ["OUTPUT_DIR"]+f"/rich/log/root_files/output_spherical_{jobid}_outb.root"
print("starting direct")
cher_dist_direct_inb    = calc_cher_residual_layers_generic(file_ele_inb, tiles_by_layer, scale_angle=False,
                                                                mode="direct",
                                                                ebpid_select=11)
cher_dist_direct_outb    = calc_cher_residual_layers_generic(file_ele_outb, tiles_by_layer, scale_angle=False,
                                                                mode="direct",
                                                                ebpid_select=11)
...
for m in mirrors_planar:
    ...
    cher_dist_planar_inb[m] = calc_cher_residual_layers_generic(
            file_ele_inb, tiles_by_layer, planar_mirror=m, ebpid_select=11, scale_angle=False,
            mode="planar"
        )
    cher_dist_planar_outb[m] = calc_cher_residual_layers_generic(
            file_ele_outb, tiles_by_layer, planar_mirror=m, ebpid_select=11, scale_angle=False,
            mode="planar"
        )
...
for m in mirrors_row1:
    for p in [12,13]:
        ...
        cher_dist_spher_row1_inb[m][p] = calc_cher_residual_layers_generic(
            file_ele_inb, tiles_by_layer, planar_mirror=p, spherical_mirror=m,
            ebpid_select=11, scale_angle=False,
            mode="spherical"
        )
        cher_dist_spher_row1_outb[m][p] = calc_cher_residual_layers_generic(
            file_ele_outb, tiles_by_layer, planar_mirror=p, spherical_mirror=m,
            ebpid_select=11, scale_angle=False,
            mode="spherical"
        )
...
def getSpreadMetricBothTorus(
    dict_direct_inb, dict_planar_inb, dict_spher_inb, dict_spher_row2_inb,
    dict_direct_outb, dict_planar_outb, dict_spher_outb, dict_spher_row2_outb,
    ...
    *,
    ...
    # direct
    a = np.asarray(dict_direct_inb[layer][tile], float)
    a = a[(a >= lo) & (a <= hi)]
    if a.size >= min_counts:
        data_topos["direct_inb"] = a
        
    a = np.asarray(dict_direct_outb[layer][tile], float)
    a = a[(a >= lo) & (a <= hi)]
    if a.size >= min_counts:
        data_topos["direct_outb"] = a

    # planar
    for key in dict_planar_inb.keys():
        a = np.asarray(dict_planar_inb[key][layer][tile], float)
        a = a[(a >= lo) & (a <= hi)]
        if a.size >= min_counts:
            data_topos[f"planar_{key}_inb"] = a
        
        a = np.asarray(dict_planar_outb[key][layer][tile], float)
        a = a[(a >= lo) & (a <= hi)]
        if a.size >= min_counts:
            data_topos[f"planar_{key}_outb"] = a

    # spherical (lower row only)
    for k1 in dict_spher_inb.keys():
        for k2 in dict_spher_inb[k1].keys():
            if layer < 203:
                a = np.asarray(dict_spher_inb[k1][k2][layer][tile], float) + spherical_shift
            else:
                a = np.asarray(dict_spher_inb[k1][k2][layer][tile], float) + spherical_shift_203
            a = a[(a >= lo) & (a <= hi)]
            if a.size >= min_counts:
                data_topos[f"spher_{k1}_{k2}_inb"] = a
            
            if layer < 203:
                a = np.asarray(dict_spher_outb[k1][k2][layer][tile], float) + spherical_shift
            else:
                a = np.asarray(dict_spher_outb[k1][k2][layer][tile], float) + spherical_shift_203
            a = a[(a >= lo) & (a <= hi)]
            if a.size >= min_counts:
                data_topos[f"spher_{k1}_{k2}_outb"] = a
    # spherical (upper 2 rows of mirrors)
    for k1 in dict_spher_row2_inb.keys():
        for k2 in dict_spher_row2_inb[k1].keys():
            if layer < 203:
                a = np.asarray(dict_spher_row2_inb[k1][k2][layer][tile], float) + spherical_shift
            else:
                a = np.asarray(dict_spher_row2_inb[k1][k2][layer][tile], float) + spherical_shift_203
            a = a[(a >= lo) & (a <= hi)]
            if a.size >= min_counts:
                data_topos[f"spher_{k1}_{k2}_inb"] = a
            if layer < 203:
                a = np.asarray(dict_spher_row2_outb[k1][k2][layer][tile], float) + spherical_shift
            else:
                a = np.asarray(dict_spher_row2_outb[k1][k2][layer][tile], float) + spherical_shift_203
            a = a[(a >= lo) & (a <= hi)]
            if a.size >= min_counts:
                data_topos[f"spher_{k1}_{k2}_outb"] = a
...
for layer in tiles_by_layer.keys():
    for tile in tiles_by_layer[layer]:
        print(layer,tile)            
        spread_together = getSpreadMetricBothTorus(
            cher_dist_direct_inb,cher_dist_planar_inb,cher_dist_spher_row1_inb,cher_dist_spher_row2_inb,
            cher_dist_direct_outb,cher_dist_planar_outb,cher_dist_spher_row1_outb,cher_dist_spher_row2_outb,
